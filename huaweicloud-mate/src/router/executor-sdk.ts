...
const SDK_INSTALLED: Set<string> = new Set();
...
export class SDKExecutor {
  ...
  private extractProduct(capabilityId: string): string | null {
    const m = capabilityId.match(/^huaweicloud\.(\w+)\./);
    return m ? m[1] : null;
  }

  private resolveMethod(capabilityId: string): string | null {
  ...
  private async ensureSDK(pkg: string): Promise<void> {
    if (SDK_INSTALLED.has(pkg)) return;
    try {
      require.resolve(pkg);
    ...
    await new Promise<void>((resolve, reject) => {
      const proc = spawn("npm", ["install", "--no-save", pkg], {
  ...
  private async callSDK(
    ...
    try {
      ...
      delete (params as any)._service;
      delete (params as any)._operation;
...
function camelAction(resource: string, action: string): string {
...
function findClientClass(mod: any, method: string): any {
  for (const key of Object.keys(mod)) {
    if (key.endsWith("Client") && typeof mod[key] === "function" && mod[key].prototype) {
      if (typeof mod[key].prototype[method] === "function") return mod[key];
    }
  }
  for (const key of Object.keys(mod)) {
    if (key.endsWith("Client") && typeof mod[key] === "function" && mod[key].prototype) {
