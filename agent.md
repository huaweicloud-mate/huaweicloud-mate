...
- ECS 已提供 `list_servers`、`get_server`、`get_job` 与受二次确认保护的 `start_servers`、`stop_servers`、`reboot_servers`；OBS 已提供 `list_buckets`、`get_bucket_metadata`、`get_bucket_location`、`list_objects`、`get_object_metadata`。完整 OpenAPI catalog 仍未导入，不能声称已覆盖 API 全量。
