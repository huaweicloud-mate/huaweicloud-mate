...
describe("Q7: HTML templates extracted from JS", () => {
  ...
  it("confirm-success.html should exist and contain 已确认", async () => {
    const fs = await import("node:fs/promises");
    const html = await fs.readFile(new URL("./templates/confirm-success.html", import.meta.url), "utf-8");
  ...
  it("confirm-expired.html should exist and contain 确认码无效", async () => {
    const fs = await import("node:fs/promises");
    const html = await fs.readFile(new URL("./templates/confirm-expired.html", import.meta.url), "utf-8");
