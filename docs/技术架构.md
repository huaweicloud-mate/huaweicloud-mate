...
开发态统一 companion 通过构建清单固定制品摘要，由 Router 以一次性子进程和私有 Node IPC 启动；审批页只在随机 loopback URL 展示规范化摘要。临时 Ed25519 私钥不落盘，receipt 绑定本次 `approvalSessionId`。最小 Router 核心已经实现 preview、受信内部 review 路径、receipt 精确绑定、dispatch 前原子消费、执行器锁定及 `OUTCOME_UNKNOWN` 后不恢复；该 review 路径不注册为第四个 Tool。

stdio MCP 已把 search、describe、普通 read execute 和危险 preview 放入真实 Tool 生命周期，但存在一个必须在 M0 Draft 冻结前解决的编排缺口：当前 input/output schema 没有定义 Router 启动内部 review 后，如何把 companion receipt 交回第二次 `execute`，而 companion 又不能暴露为第四个 Tool。本轮不通过隐藏字段、自动批准或未定义 MCP metadata 绕过该问题。四宿主 receipt 交接和进程/loopback 隔离验证仍是后续门禁。
