...
- 首版内置本地 OBS provider，并保留后续华为云产品 MCP 接入口；
- 本地 KooCLI；
- 华为云资源操作 Skills；
- 四类 Agent 的自动配置模板；
- 固定三个工具的本地 Tool Router MCP。

首版不接入真实产品 MCP，由内置本地 OBS provider 提供已实现能力，KooCLI 补足后续映射。真实产品 MCP 到位后仍按静态内置、MCP 优先的既有接口接入，不改变三工具协议。

## 2. 精简原则

1. **单一 npm 包**：首版不拆分多 package 控制平面。
2. **固定三个工具**：Agent 只看到 search、describe、execute。
3. **内置 Provider**：首版本地 OBS provider 与未来产品 MCP endpoint/元数据都随 npm 发版，不建设动态 Registry。
4. **静态执行前路由**：首版使用本地 OBS provider 与 KooCLI；未来产品 MCP 支持且健康时优先 MCP，否则只在执行前选择 KooCLI。
...
公开源码、Git 历史和 npm tarball 分别通过源码卫生、secret 扫描和 pack allowlist 门禁；正式文档与构建产物不得包含开发 workspace、真实用户 profile、内部 endpoint 或停用规划文件。

ADR-0011 已实现该运行时核心：构建产出自包含 Router/CLI 与 companion bundle，并为完整 `dist` 生成路径、大小和 SHA-256 清单；Installer 先写入 `versions/<pluginVersion>` staging、复验后原子改名，最后更新 `current/active-runtime.json`。稳定 `current/hcloud-agent.mjs` 在每次启动前校验活动版本的完整清单，再导入 Router。ADR-0033 又在正式 CLI 安装和卸载状态读取边界接入用户级 runtime 递归权限门禁，覆盖版本、状态、恢复/备份证据与私有 KooCLI。ADR-0037 保留校验时读取的 `runtime/cli.js` 字节并从内存 module 加载；ADR-0038 同样经私有 stdin 把已验签 companion 入口字节交给固定 bootstrap；ADR-0039 再把同轮验签的七份审批契约文本交给父端 verifier 与子端 signer 内存编译；ADR-0041 禁用 companion 的 SIGUSR1 inspector、拒绝 Node 注入环境并把随机 loopback 页面收敛为一次读取。入口与审批 schema 都不会在摘要校验后按路径重读。摘要和 ACL 仍不能抵抗同账号攻击者同时替换 launcher、指针、manifest 和全部制品，也不能证明浏览器启动 URL 与 OS 调试对同账号 Agent 不可见；正式真实性仍依赖发布 provenance 和四宿主真实隔离。
...
四宿主均已实现这一入口。Codex/Claude 同时管理 marketplace/CLI activation 及结果未知恢复；OpenCode/码道因配置固定指向 stable launcher，升级不重写 JSON/JSONC，只按 ownership/hash 替换 Canonical Skill，再以 active pointer 和 install-state CAS 提交。配置宿主的严格 marker 可收敛旧/候选 state、Skill 和随机 rollback quarantine，细节见 ADR-0034。
...
开发态 stdio MCP Server 已使用官方 TypeScript SDK 的稳定 v1 接口实现，并通过真实子进程协议测试确认只列出上述三个工具。随包静态 catalog 同时包含不访问云资源的 reference 能力和首版本地 OBS provider；reference 只验证契约路径，本地 OBS 则以固定 endpoint、短会话和统一审批覆盖列桶、有界敏感文本读取、建桶和空桶删除。它不冒充 Streamable HTTP 产品 MCP，但作为首版内置能力仍必须完成真实最小权限账号验收。
...
Agent 必须把摘要展示给用户。只有固定受信审批签发器观察到明确用户审批后，才能生成 `approvalReceipt`；Agent、Prompt、Skill 和普通 Tool 参数不能自行签发、读取或转发。第二次 `execute` 重复相同 capability、参数和 scope，并只额外携带 `previewId`。

开发态统一 companion 通过构建清单固定制品摘要，由 Router 以一次性子进程和私有 Node IPC 启动；审批页只在随机 loopback URL 展示规范化摘要。临时 Ed25519 私钥不落盘，receipt 绑定本次 `approvalSessionId`。第二次调用在 Router 内部完成 review、回执验签、凭证复核和 dispatch 前原子消费；同一 preview 的并发调用共享一次 review，只有一个调用可以 dispatch。该 review 路径不注册为第四个 Tool，receipt 不进入 Tool 输入、输出或 Agent 上下文。

stdio MCP 已把 search、describe、普通 read execute 和危险两阶段 execute 放入真实 Tool 生命周期。ADR-0010 已冻结交接方式：公开 schema 不接受 receipt；第二次调用只提交 `previewId`，并阻塞等待 Router 内部 companion 结果。用户拒绝、账号或凭证代次变化会消费 preview；companion 进程/UI 故障不会自动批准，可在 preview 未过期时显式重试。四宿主安装目录真实性和进程/loopback 隔离验证仍是后续门禁。
...
- Adapter 只把 credential generation 和预期账号交给随包受信调用边界，不接收 AK/SK；ADR-0050 授权的 invoker 在 dispatch 前从 CredentialStore 读取当前永久 AK/SK，以绝对路径、`shell=false`、最小环境和无 profile 的官方 argv 模式启动 KooCLI；
- 规范化 JSON 结果和错误；
- 设置超时、取消和输出大小上限；
...
该 JSONL 基础已由真实 stdio Router 接入：参数和结果只记录 canonical SHA-256 摘要；日志文件使用当前用户权限、单行上限和有界轮转。dispatch 意图必须在执行器调用前落盘，完成/失败事件在 dispatch 后尽力写入，避免日志介质故障诱导副作用重试。详细边界见 ADR-0032。
...
4. 内置本地 OBS provider 能通过进程内短会话执行已声明的云资源操作；
5. 未覆盖能力能够在执行前选择受信 KooCLI Adapter；只有 credentials、兼容 KooCLI、静态 mapping 和授权 argv invoker 同时可用时才 dispatch；
6. 兼容的预装 KooCLI 可复用，不兼容时自动安装并验证私有固定版本；
7. 创建、修改、删除、IAM、网络、计费和敏感读取必须携带可信审批凭据；
8. 确认后执行器锁定，失败或结果未知时不自动切换；
9. AK/SK 不进入 Agent 上下文、工具参数、workspace 或插件日志；唯一命令行例外是 ADR-0050 授权的直接 KooCLI 子进程 argv；
...
以下内容不改变架构。reference provider 和统一受信 companion 已解除本地工程开发阻塞；ADR-0048 已把真实产品 MCP 移出首版，真实发布前仍必须明确：

- 公共 npm scope 与发布身份；
- 统一受信审批 companion 的实现验证、私钥保护和责任人；
- 首个真实 KooCLI 产品 capability mapping 与最小权限账号验收；制品和授权 argv invoker 已由 ADR-0048/0050 绑定；
- 3～5 个端到端验收场景；
- `auth set` 使用的固定只读账号身份校验能力及责任人。

真实产品 MCP 清单、同源 endpoint、capability 风险/schema digest 与 credential-session 责任人保留为后续版本接入输入。
