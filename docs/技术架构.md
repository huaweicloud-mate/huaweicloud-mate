...
开发态统一 companion 通过构建清单固定制品摘要，由 Router 以一次性子进程和私有 Node IPC 启动；审批页只在随机 loopback URL 展示规范化摘要。临时 Ed25519 私钥不落盘，receipt 绑定本次 `approvalSessionId`。最小 Router 核心已经实现 preview、受信内部 review 路径、receipt 精确绑定、dispatch 前原子消费、执行器锁定及 `OUTCOME_UNKNOWN` 后不恢复；该 review 路径不注册为第四个 Tool。stdio MCP 三工具服务与四宿主进程/loopback 隔离验证仍是后续门禁。
