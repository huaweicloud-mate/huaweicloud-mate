...
6. **最小强制安全边界**：Router 内置风险门禁、一次性确认令牌、脱敏和超时，不建设独立 Policy Engine。
...
`read` 能力在校验参数和执行器后直接执行。

`write`、`destructive`、`privileged`、`cost` 能力首次调用返回：
...
Agent 必须把摘要展示给用户。用户明确确认后，Agent 再次调用 `execute` 并携带 token。token 必须：

- 绑定 capability、参数摘要、执行器、当前账号、region/project 和过期时间；
- 单次使用；
- 任一绑定字段变化时失效；
- 不包含 AK/SK 或可逆敏感数据。
...
product: ecs
version: 1.0.0
dataPlane: https://example.huaweicloud.com/mcp
credentialSession: https://example.huaweicloud.com/credential-sessions
...
- `auth set` 覆盖更新后立即撤销全部旧远程 session；
- `auth remove` 删除凭证并撤销 session。
...
sequenceDiagram
  ...
  P-->>C: session ID、账号信息与过期时间
  C->>C: 校验账号并清除临时凭证副本
  C->>P: Streamable HTTP 调用及 session ID
  P-->>C: 结构化结果与 request ID
  C-->>A: 脱敏后的执行结果
  Note over C,P: 过期、凭证更新、auth remove、断连或服务重启时清理 session
```
...
| 级别 | 示例 | Router 行为 |
|---|---|---|
| `read` | 列表、查询、状态 | 校验后直接执行 |
| `write` | 创建、修改、配置 | 返回摘要并要求确认 |
| `destructive` | 删除、解绑、停止 | 返回突出展示的破坏性摘要并要求确认 |
| `privileged` | IAM、权限、网络暴露 | 返回权限与暴露范围并要求确认 |
| `cost` | 创建计费资源、扩容 | 返回计费相关摘要并要求确认 |
...
以下内容不改变架构，但必须在 M0 结束前明确：

- 公共 npm scope 与发布身份；
- 首发产品 MCP 清单和同源 endpoint；
- 每个产品 MCP 的 capability/risk 元数据；
