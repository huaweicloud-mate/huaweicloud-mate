...
开发态统一 companion 通过构建清单固定制品摘要，由 Router 以一次性子进程和私有 Node IPC 启动；审批页只在随机 loopback URL 展示规范化摘要。临时 Ed25519 私钥不落盘，receipt 绑定本次 `approvalSessionId`。该 launcher 尚未接入真实 Router execute，四宿主进程/loopback 隔离验证仍是发布门禁。
