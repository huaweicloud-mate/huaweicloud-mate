...
开发态 stdio MCP Server 已使用官方 TypeScript SDK 的稳定 v1 接口实现，并通过真实子进程协议测试确认只列出上述三个工具。随包静态开发 catalog 只包含不访问云资源的 reference 能力；它用于验证 search、describe、普通 read execute 和危险 preview，不进入正式 Provider 清单。
