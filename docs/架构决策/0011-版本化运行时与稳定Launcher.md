...
它不能抵抗拥有当前账号任意文件写权限的攻击者同时替换稳定 launcher、active pointer、版本目录和全部摘要。正式发布仍必须依赖 npm provenance/制品签名、用户目录 ACL、安装目录写入隔离，以及四宿主对同账号 Agent 进程能力的验证。完成这些门禁前不得把本地摘要描述为抵抗同账号任意代码执行的“真实性证明”。本 ADR 决策时存在的主 CLI 摘要校验到模块加载窗口已由 ADR-0037 关闭；companion 等独立进程边界仍需单独处理。
