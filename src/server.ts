...
const tools = [
  { name: "huaweicloud_discover", description: "Search ECS/OBS child MCP capability modules without loading every schema. Call at most once per requested service. If a non-empty query returns the koocli fallback, do not retry discovery or guess MCP service names: provision koocli and use huaweicloud_call with service=koocli, operation=run. Its command array contains arguments after hcloud, not the hcloud executable.", inputSchema: { type: "object", properties: { query: { type: "string" } } } },
