 ...
 *
 * 首版：MCP (通过 stdin/stdout 调 MetaMCP) + KooCLI (子进程)
 * 二期：SDK + Terraform
 */
...
import { KooCLI_BINARY_PATH } from "./koocli-installer.js";

/** Mock MCP Server 路径 — 本地联调用 */
const MOCK_ECS_SERVER = join(__dirname, "..", "..", "dist", "mock", "ecs-mock-server.js");

/** MCP Server 注册表 */
type MCPServerConfig = { bin: string; args: string[]; env?: Record<string, string> };

const MCP_SERVERS: Record<string, MCPServerConfig> = {
  // Mock（本地联调）
  "ecs-mock-server": { bin: "node", args: [join(__dirname, "..", "mock", "ecs-mock-server.js")] },

  // 产品部开发的真实 MCP Server（来自 huaweicloud-mcp 仓库）
  "obs-server":  { bin: "python3", args: [join(__dirname, "..", "..", "..", "huaweicloud-mcp", "src", "obs-server", "server.py")] },
  "nat-server":  { bin: "python3", args: ["-m", "huaweicloud.nat.server"], 
                    env: { PYTHONPATH: join(__dirname, "..", "..", "..", "huaweicloud-mcp", "src", "nat-server") } },
};
...
export class ExecutorRouter {
  async execute(
    ...
    if (executor === "mcp") {
      return this.executeMCP(capabilityId, params, credentials, correlationId, startTime, toolOverride);
    }

    if (executor === "koocli") {
      return this.executeKooCLI(capabilityId, params, credentials, correlationId, startTime);
    }
  ...
  private async executeMCP(
    ...
    correlationId: string,
    startTime: number,
    toolOverride?: string   // 从 Catalog 中取的真实 tool 名 (如 obs.list_buckets)
  ): Promise<ExecutionResult> {
  ...
  private resolveMCPServer(
    capabilityId: string,
    toolOverride?: string
  ): { bin: string; args: string[]; env?: Record<string,string>; tool: string } | null {
  ...
  private async callMCPServer(
    server: { bin: string; args: string[]; env?: Record<string,string>; tool: string },
    params: Record<string, any>,
    credentials: CredentialConfig,
    correlationId: string,
    startTime: number
    ...
    return new Promise((resolve) => {
      const env = {
        ...
        HUAWEICLOUD_REGION: credentials.huaweicloud_region || "cn-north-4",
      };

      const proc = spawn(server.bin, server.args, {
        env,
  ...
  private async executeKooCLI(
    ...
    const args = [
      KooCLI_BINARY_PATH, service, operation,
      "--cli-output=json",
    ...
    for (const [key, value] of Object.entries(params)) {
      ...
      if (key === "service" || key === "operation") continue;
      args.push(`--${key}=${value}`);
    }
    ...
    return new Promise((resolve) => {
      const proc = spawn(KooCLI_BINARY_PATH, args.slice(1), {
        env,
        stdio: ["pipe", "pipe", "pipe"],
        timeout: 30_000,
      });
  ...
  private redact(text: string, creds: CredentialConfig): string {
    let result = text;
  ...
  async healthCheck(): Promise<Record<string, { status: string; message?: string }>> {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        const proc = spawn(KooCLI_BINARY_PATH, ["--version"], {
          stdio: ["pipe", "pipe", "pipe"],
          timeout: 5000,
        });
        proc.stdin.write("y\n");  // 接受隐私政策
  ...
  private async probeMCPServer(name: string, cfg: MCPServerConfig): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      proc.on("close", (code) => {
        clearTimeout(timer);
        if (code === 0) reject(new Error("Server exited without response"));
