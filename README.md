...
当前工程仅用于开发和契约验证，尚不能作为可安装的正式插件或 MCP Server 使用。请勿提供真实 AK/SK，也不要通过环境变量、工具参数或日志传递凭证。

早期 OBS-only 原型已经从活动分支退役，仍可在 Git 历史中追溯。当前先推进单包骨架、契约注册表和 doctor；真实产品 MCP、固定 KooCLI 制品、账号身份校验和真实端到端场景仍是后续发布门禁。
